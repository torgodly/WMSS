                            ...
                            @foreach($warehouses as $warehouse)
                                ...
                                <tr class="bg-white">
                                    ...
                                    <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-900 flex  gap-3">
                                        <x-secondary-button :link="route('warehouse.index', ['ID'=> $warehouse->id])">
                                            Edit
                                        </x-secondary-button>
                        ...
                        <div
                            ...
                            <div class="">
                                <div class="flex  justify-between items-center ">
                                    ...
                                    <form action="{{route('warehouse.destroy', $warehousesEdit->id)}}" method="post">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <p>Edit modal</p>
                                <form action="{{route('warehouse.update', $warehousesEdit->id)}}" method="post" class="mt-6 space-y-6">
                                    @csrf
                                    @method('PATCH')
                                    <div>
                                        <x-input-label for="name" :value="__('Name')"/>
                                    ...
                                    <div>
                                        ...
                                        <select name="user_id"
                                                class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                                            @foreach($users as $user)
                                                <option @if($warehousesEdit->user_id == $user->id) selected @endif value="{{$user->id}}">{{$user->name}}</option>
                                            @endforeach
                                        </select>
                                        <x-input-error class="mt-2" :messages="$errors->get('user_id')"/>
                                    </div>
